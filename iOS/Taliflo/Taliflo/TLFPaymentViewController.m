...
@interface TLFPaymentViewController ()

@property (nonatomic, strong) CustomKeyboard *customKeyboard;

@end

@implementation TLFPaymentViewController
...
- (IBAction)completePayment:(id)sender
{
    ...
    onCompletion = ^void(STPToken *token, NSError *error) {
        if (error) {
            ...
            [TLFAlert okAlertForViewController:self withTitle:@"Payment Error" message:[error localizedDescription]];
        } else {
            // Success
            NSLog(@"%@", token.tokenId);
