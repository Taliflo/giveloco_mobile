...
- (UIBarButtonItem *)getNextPrevButtons:(BOOL)prevEnabled :(BOOL)nextEnabled {
    ...
    UISegmentedControl *tabNavigation = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:[UIImage imageNamed:@"UIButtonBarArrowUp"], [UIImage imageNamed:@"UIButtonBarArrowDown"], nil]];
    [tabNavigation setEnabled:prevEnabled forSegmentAtIndex:0];
    [tabNavigation setEnabled:nextEnabled forSegmentAtIndex:1];
    [tabNavigation setWidth:50.0f forSegmentAtIndex:0];
    [tabNavigation setWidth:50.0f forSegmentAtIndex:1];
...
- (UIBarButtonItem *)getDoneButton {
    return [[UIBarButtonItem alloc] initWithTitle:@"Close" style:UIBarButtonItemStylePlain target:self action:@selector(userClickedDone:)];
}
